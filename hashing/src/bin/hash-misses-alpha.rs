...
fn main() {
  Algorithm::new("Hashing Misses", calculate)
    .with_case(
      Case::new("Misses")
        ...
        .step_size(1)
        .plots(vec![
          Plot::new(|alpha| 0.5 * (1.0 + 1.0 / (1.0 - (alpha as f64 / 100.0)).powi(2)), RED, "Expected")
        ])
    )
    .run()
    .plot("hashing-misses")
...
fn calculate(arr: &mut [i32]) -> f64 {
  ...
  let (table, to_test) = create_table_with_load(load_factor);
...
fn create_table_with_load(expected_load: f64) -> (HashTable<i32, i32>, i32) {
  ...
  while table.get_load_factor() < expected_load {
    println!("Factor: {}, to match: {}", table.get_load_factor(), expected_load);
