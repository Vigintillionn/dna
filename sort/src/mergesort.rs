...
pub fn sort<T: Ord + Copy>(arr: &mut [T], ret_comparisons: bool) -> f64 {
  ...
  let mut accesses: f64 = 0.0;
  merge_sort(arr, &mut aux, 0, arr.len() - 1, &mut comparisons, &mut accesses);
...
fn merge_sort<T: Ord + Copy>(arr: &mut [T], aux: &mut Vec<T>, low: usize, high: usize, comparisons: &mut f64, accesses: &mut f64) -> f64 {
  // let mut comparisons: usize = 0;
  // Base case: if low is greater than or equal to high, the array is sorted
  if low < high {
    let mid = low + (high - low) / 2;
    // Recursively sort the left and right halves
    merge_sort(arr, aux, low, mid, comparisons, accesses);
    merge_sort(arr, aux, mid + 1, high, comparisons, accesses);
    merge(arr, aux, low, mid, high, comparisons, accesses);
  }
  *comparisons
}

fn merge<T: Ord + Copy>(arr: &mut [T], aux: &mut Vec<T>, low: usize, mid: usize, high: usize, comparisons: &mut f64, accesses: &mut f64)  {
